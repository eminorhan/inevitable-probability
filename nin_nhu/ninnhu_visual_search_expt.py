...
nnn              = np.ceil(np.logspace(.5,2.2,14))
...
if __name__ == '__main__':
    ...
    tr_cond               = 'all_gains'
    test_cond             = 'all_gains'
    generator             = VisualSearchTaskFFWD(max_iter=300001, batch_size=512, n_loc=4, n_in=n_in, n_out=1, kappa=2.0, sT=0.0, n_rm=100, tr_cond=tr_cond)
    l_out, l_in_hid       = model(input_var, batch_size=generator.batch_size, n_in=generator.n_loc*generator.n_in, n_out=generator.n_out, n_hid=n_hid)
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('vs_nin%i_nhu%i_jobidx%i.mat'%(n_in,n_hid,job_idx), {'inf_loss':inf_loss, 'inf_loss_vec':np.asarray(inf_loss_vec), 'success':success } )      