...
if __name__ == '__main__':
    ...
    tr_cond               = 'all_gains'
    test_cond             = 'all_gains'
    generator             = CoordinateTransformationTaskFFWD(max_iter=50001, batch_size=100, n_in=50, n_out=1, sigma_sq=100.0, tr_cond=tr_cond)
    test_generator        = CoordinateTransformationTaskFFWD(max_iter=2501,  batch_size=100, n_in=50, n_out=1, sigma_sq=100.0, tr_cond=test_cond)    
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ct_abserr_testfrmse_allgains_run%i.mat'%job_idx, {'frac_rmse_test':frac_rmse_test, 'opt_vec':np.asarray(opt_s_vec), 'net_vec':np.asarray(ex_pred_vec) })      
    sio.savemat('ct_abserr_everything_allgains_run%i.mat'%job_idx, {'W_hid': lasagne.layers.get_all_param_values(l_out)[0], 
