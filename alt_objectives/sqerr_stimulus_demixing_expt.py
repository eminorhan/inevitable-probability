...
if __name__ == '__main__':
    ...
    generator             = StimulusDemixingTaskFFWD(W_mix, f_I, f_b, max_iter=100001, batch_size=100, n_in=10, n_out=4, nmc = 250, tr_cond=tr_cond)
    test_generator        = StimulusDemixingTaskFFWD(W_mix, f_I, f_b, max_iter=2501,  batch_size=100, n_in=10, n_out=4, nmc = 250, tr_cond=test_cond)
    l_out, l_rec          = model(input_var, batch_size=generator.batch_size, n_in=generator.n_out*generator.n_in, n_out=generator.n_out, n_hid=200)
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    ex_inp_vec = np.reshape(ex_inp_vec,(-1,40))    
            
    # SAVE TRAINED MODEL  
    sio.savemat('sd_sqerr_testinfloss_twogains_run%i.mat'%job_idx, {'inf_loss_test':inf_loss_test, 'opt_vec':opt_vec, 'net_vec':net_vec})      
#    sio.savemat('sd_everything_allgains_run%i.mat'%job_idx, {'W_hid': lasagne.layers.get_all_param_values(l_out)[0], 
