...
if __name__ == '__main__':
    ...
    tr_cond               = 'two_gains'
    test_cond             = 'all_gains'
    generator             = VisualSearchTaskFFWD(max_iter=300001, batch_size=500, n_loc=4, n_in=20, n_out=1, kappa=2.0, sT=0.0, n_rm=100, tr_cond=tr_cond)
    test_generator        = VisualSearchTaskFFWD(max_iter=2501,  batch_size=500, n_loc=4, n_in=20, n_out=1, kappa=2.0, sT=0.0, n_rm=100, tr_cond=test_cond)
    l_out, l_in_hid       = model(input_var, batch_size=generator.batch_size, n_in=generator.n_loc*generator.n_in, n_out=generator.n_out, n_hid=200)
...
#    l2_penalty            = lasagne.regularization.regularize_layer_params(all_layers, lasagne.regularization.l2) * 1e-6
    ...
    rec_layer_fn          = theano.function([input_var], lasagne.layers.get_output(l_in_hid, get_details=True), allow_input_downcast=True)
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    ex_inp_vec = np.reshape(ex_inp_vec,(-1,80))    
            
    # SAVE TRAINED MODEL  
    sio.savemat('vs_sqerr_testinfloss_twogains_run%i.mat'%job_idx, {'inf_loss_test':inf_loss_test, 'opt_vec':opt_vec, 'net_vec':net_vec})      
#    sio.savemat('vs_everything_allgains_run%i.mat'%job_idx, {'W_hid': lasagne.layers.get_all_param_values(l_out)[0], 
