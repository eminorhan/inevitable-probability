...
from generators import KalmanFilteringTask
...
if __name__ == '__main__':
    ...
    tr_cond               = 'two_gains'
    test_cond             = 'all_gains'
    generator             = KalmanFilteringTask(max_iter=250001, batch_size=100, n_in=50, n_out=1, stim_dur=25, sigtc_sq=4.0, signu_sq=1.0, gamma=0.1, tr_cond=tr_cond)
    test_generator        = KalmanFilteringTask(max_iter=2501, batch_size=100, n_in=50, n_out=1, stim_dur=25, sigtc_sq=4.0, signu_sq=1.0, gamma=0.1, tr_cond=test_cond)
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ei_kf_testinfloss_twogains_run%i.mat'%job_idx, {'frac_rmse_test':frac_rmse_test, 'opt_vec':np.asarray(opt_s_vec), 'net_vec':np.asarray(ex_pred_vec) } )      
#    sio.savemat('ei_kf_everything_allgains_run%i.mat'%job_idx, {'W_in': W_in, 
#                                                                'W_hid': W_hid,
