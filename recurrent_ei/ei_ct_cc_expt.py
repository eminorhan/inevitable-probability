...
if __name__ == '__main__':
    ...
    lasagne.layers.set_all_param_values(l_fix, [np.zeros((1,200)).astype('float32'),
                                                sio.loadmat('ei_ct_everything_allgains_run1.mat')['W_in'].astype('float32'), 
                                                sio.loadmat('ei_ct_everything_allgains_run1.mat')['W_hid'].astype('float32'), 
                                                sio.loadmat('ei_ct_everything_allgains_run1.mat')['b_hid'].flatten().astype('float32')])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ei_ct_cc_testinfloss_twogains_run%i.mat'%job_idx, {'frac_rmse_test':frac_rmse_test, 'opt_vec':np.asarray(opt_s_vec), 'net_vec':np.asarray(ex_pred_vec) } )      