...
from generators import VisualSearchTask
...
def model(input_var, batch_size=1, n_in=100, n_out=1, n_hid=200, ei_ratio=0.8):
    ...
    l_dense      = DenseEILayer(l_shp, ei_ratio, num_units=n_out, W=lasagne.init.GlorotNormal(0.1), nonlinearity=lasagne.nonlinearities.sigmoid, leak=1.0, diagonal=True)
...
if __name__ == '__main__':
    ...
    tr_cond               = 'two_gains'
    test_cond             = 'all_gains'
    generator             = VisualSearchTask(max_iter=250001, batch_size=1000, n_loc=4, n_in=20, n_out=1, stim_dur=10, kappa=2.0, sT=0.0, n_rm=100, tr_cond=tr_cond)
    test_generator        = VisualSearchTask(max_iter=2501, batch_size=1000, n_loc=4, n_in=20, n_out=1, stim_dur=10, kappa=2.0, sT=0.0, n_rm=100, tr_cond=test_cond)
    
    l_out, l_rec          = model(input_var, batch_size=generator.batch_size, n_in=generator.n_loc*generator.n_in, n_out=generator.n_out, n_hid=200, ei_ratio=0.8)
    ...
    loss                  = T.mean(lasagne.objectives.binary_crossentropy(pred_var[:,-1,-1], target_var[:,-1,-1])) + l2_penalty
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    for i, (example_input, example_output, p) in generator:
        ...
        opt_vec.append(p)
        net_vec.append(example_prediction[:,-1,-1])
        if i % 500 == 0:
    ...
    ex_inp_vec = np.reshape(ex_inp_vec,(-1, test_generator.stim_dur, 80))

    # Prepare the actual connectivity matrices
    D_in  = np.diag( np.concatenate( (np.ones(64),-np.ones(16)) ) ) 
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ei_vs_testinfloss_twogains_run%i.mat'%job_idx, {'inf_loss_test':inf_loss_test, 'opt_vec':opt_vec, 'net_vec':net_vec})      
#    sio.savemat('ei_vs_everything_allgains_run%i.mat'%job_idx, {'W_in':  W_in, 
#                                                                'W_hid': W_hid,
