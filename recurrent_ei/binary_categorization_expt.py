...
if __name__ == '__main__':
    ...
    tr_cond               = 'all_gains'
    test_cond             = 'all_gains'
    generator             = BinaryCategorizationTask(max_iter=100001, batch_size=100, n_in=50, n_out=1, stim_dur=10, sigma_sq=100.0, tr_cond=tr_cond)
    test_generator        = BinaryCategorizationTask(max_iter=2501,  batch_size=100, n_in=50, n_out=1, stim_dur=10, sigma_sq=100.0, tr_cond=test_cond)

    l_out, l_rec          = model(input_var, batch_size=generator.batch_size, n_in=generator.n_in, n_out=generator.n_out, n_hid=200, ei_ratio=0.8)
    # The generated output variable and the loss function
    pred_var              = T.clip(lasagne.layers.get_output(l_out), 1e-6, 1. - 1e-6)
    loss                  = T.mean(lasagne.objectives.binary_crossentropy(pred_var[:,-1,-1], target_var[:,-1]))
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    ex_inp_vec = np.reshape(ex_inp_vec,(-1, test_generator.stim_dur, 50))    

    # Prepare the actual connectivity matrices
    D_in  = np.diag( np.concatenate( (np.ones(40),-np.ones(10)) ) )
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ei_bc_testinfloss_allgains_run%i.mat'%job_idx, {'inf_loss_test':inf_loss_test, 'opt_vec':opt_vec, 'net_vec':net_vec})      
    sio.savemat('ei_bc_everything_allgains_run%i.mat'%job_idx, {'W_in':  W_in, 
                                                                'W_hid': W_hid,
