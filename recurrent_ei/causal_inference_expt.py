...
from generators import CausalInferenceTask
...
if __name__ == '__main__':
    ...
    tr_cond               = 'all_gains'
    test_cond             = 'all_gains'
    generator             = CausalInferenceTask(max_iter=100001, batch_size=100, n_in=50, n_out=1, stim_dur=10, sigma_sq=100.0, tr_cond=tr_cond)
    test_generator        = CausalInferenceTask(max_iter=2501, batch_size=100, n_in=50, n_out=1, stim_dur=10, sigma_sq=100.0, tr_cond=test_cond)
    ...
    loss                  = T.mean(lasagne.objectives.binary_crossentropy(pred_var[:,-1,-1], target_var[:,-1,-1])) 
...
#    lasagne.layers.set_all_param_values(l_rec,[npzfile_lout['arr_0'],npzfile_lout['arr_1'],npzfile_lout['arr_2'],npzfile_lout['arr_3'],npzfile_lout['arr_4']])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ei_ci_testinfloss_allgains_run%i.mat'%job_idx, {'inf_loss_test':inf_loss_test, 'opt_vec':opt_vec, 'net_vec':net_vec})      
    sio.savemat('ei_ci_everything_allgains_run%i.mat'%job_idx, {'W_in':  W_in, 
                                                                'W_hid': W_hid,
