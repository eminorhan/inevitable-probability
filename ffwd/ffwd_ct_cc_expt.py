...
if __name__ == '__main__':
    ...
    updates               = lasagne.updates.adam(loss, trainable_params, learning_rate=0.0005)
    ...
    lasagne.layers.set_all_param_values(l_fix, [sio.loadmat('ct_everything_allgains_run3.mat')['W_hid'].astype('float32'), 
                                                sio.loadmat('ct_everything_allgains_run3.mat')['b_hid'].flatten().astype('float32')])
    ...
    # SAVE TRAINED MODEL  
    sio.savemat('ctcc_testfrmse_allgains_run%i.mat'%job_idx, {'frac_rmse_vec':np.asarray(frac_rmse_vec), 'frac_rmse_test':frac_rmse_test, 'opt_vec':np.asarray(opt_s_vec), 'net_vec':np.asarray(ex_pred_vec) } )      
